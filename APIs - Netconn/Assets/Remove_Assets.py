...
urlCofre            = f'https://{cofrePorto}/BeyondTrust/api/public/v3'
...
def Remove_Asset_by_id():
    with open(r'C:\Users\wsantos\Documents\APIs - Netconn\Remove\Assets.csv') as csvfile:
        ...
        for row in reader:
            sleep(0.5)
            AssetID     = row['AssetID'] # Seleciona conteudo na coluna AssetID
            AssetName   = row['AssetName']
        
            urlAsset = urlCofre + f"/Assets/{AssetID}"
            remove_asset = session.delete(url = urlAsset, verify=False)

            print(f"Status Code {remove_asset} | Removido Asset: {AssetName}")
####################################################################
