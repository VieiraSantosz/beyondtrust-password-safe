...
def Get_AddressGroup():
    ...
    for i in adressGroup.json():
        ...
        print(f"ID do Address Group - {addressID} | Nome do Addres Group - {addressName}") 
    ...
    address     = session.post(url = urlAdress, verify=False)
        
    for i in address.json():
        nome_adress   = i['Value']
        
        print(f"Máquina - {nome_adress}")
        
        with open ("Get\Máquinas.csv", "a+") as file:
            file.write("\n" + nome_adress)
